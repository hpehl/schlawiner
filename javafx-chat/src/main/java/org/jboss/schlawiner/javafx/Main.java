package org.jboss.schlawiner.javafx;
...
public class Main extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        StreamObserver<ClientMessage> chat = chatService.chat(new StreamObserver<ServerMessage>() {
            @Override
            public void onNext(ServerMessage value) {
                Platform.runLater(() -> {
                    messages.add(value.getMessage().getPlayer() + ": " + value.getMessage().getMessage());
        ...
        send.setOnAction(e -> {
            ClientMessage clientMessage = ClientMessage.newBuilder()
                .setPlayer(name.getText())
                .setMessage(message.getText())
                .build();
            chat.onNext(clientMessage);
            message.setText("");
        });
